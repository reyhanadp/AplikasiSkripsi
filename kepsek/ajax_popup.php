...
$query_notif = "SELECT id_notifikasi,tb_siswa.nama,tb_siswa.foto,pesan_notif FROM `tb_notifikasi` JOIN tb_siswa ON tb_notifikasi.nis=tb_siswa.nis WHERE id_notifikasi not in (SELECT id_notifikasi from tb_status where id_user='" . $_SESSION[ 's_nuptk' ] . "') AND pesan_notif != 'keluar rumah'";
