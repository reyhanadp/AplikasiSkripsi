...
while ( $data = mysqli_fetch_assoc( $res_guru ) ) {
	$query_notifikasi = "SELECT id_notifikasi,tb_siswa.lat,tb_siswa.longitude,tb_siswa.nama,tb_notifikasi.pesan_notif,tb_kelas.kelas,tb_kelas.tingkatan FROM `tb_notifikasi` JOIN tb_siswa ON tb_notifikasi.nis=tb_siswa.nis JOIN tb_kelas ON tb_siswa.id_kelas=tb_kelas.id_kelas WHERE id_notifikasi not in (SELECT id_notifikasi from tb_status where id_user='" . $data[ 'nuptk' ] . "') AND pesan_notif!='keluar rumah'";
