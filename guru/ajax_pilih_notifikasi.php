...
$sql_update_status = "UPDATE `tb_status` SET `status`='1' WHERE `id_status`='".$_POST['idStatus']."'";
$res_update_status = mysqli_query($link,$sql_update_status);


$query_ambil_konfirmasi = "select tb_notifikasi.nis, nama, foto , kelas, tingkatan, tb_notifikasi.status, tb_notifikasi.pesan_notif, id_notifikasi, tb_notifikasi.nuptk, waktu, tb_siswa.lat, tb_siswa.longitude from tb_notifikasi join tb_siswa on tb_notifikasi.nis=tb_siswa.nis JOIN tb_kelas on tb_siswa.id_kelas=tb_kelas.id_kelas where tb_notifikasi.id_notifikasi='".$_POST['idNotifikasi']."'";
