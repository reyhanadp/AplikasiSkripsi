...
if ( isset( $_GET[ 'nis' ] ) ) {
	...
	$sql_siswa = "select tb_siswa.nama,tb_siswa.baterai,tb_siswa.nis,tb_siswa.lat,tb_siswa.longitude,tb_siswa.foto,tb_siswa.status,tb_kelas.kelas,tb_kelas.jam_masuk,tb_kelas.jam_keluar from tb_siswa JOIN tb_kelas ON tb_siswa.id_kelas=tb_kelas.id_kelas where tb_siswa.lat IS NOT NULL AND tb_siswa.status != 1 AND id_orangtua = '".$_SESSION['s_id_orangtua']."'";
}
