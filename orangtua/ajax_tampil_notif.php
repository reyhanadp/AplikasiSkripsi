...
$sql_ambil_notif = "SELECT tb_status.id_status,tb_notifikasi.id_notifikasi,tb_notifikasi.nis,tb_notifikasi.pesan_notif,tb_notifikasi.waktu,tb_siswa.foto,tb_siswa.nama,TIMESTAMPDIFF(minute,tb_notifikasi.waktu,now()) as menit, DATEDIFF(now(), waktu) AS hari, TIMESTAMPDIFF(MONTH, waktu, now()) AS bulan  FROM `tb_notifikasi` join tb_status on tb_notifikasi.id_notifikasi=tb_status.id_notifikasi JOIN tb_siswa ON tb_notifikasi.nis=tb_siswa.nis WHERE tb_status.status=0 AND tb_status.id_user='" . $_SESSION[ 's_id_orangtua' ] . "'";
