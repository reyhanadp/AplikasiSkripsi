...
$sql_ambil_pesan = "SELECT `id_pesan`, `id_pengirim`, `isi_pesan`, `waktu`,TIMESTAMPDIFF(minute,waktu,now()) as menit, DATEDIFF(now(), waktu) AS hari, TIMESTAMPDIFF(MONTH, waktu, now()) AS bulan FROM `tb_pesan` WHERE `id_penerima`='" . $_SESSION[ 's_id_orangtua' ] . "' AND `status`='0' ORDER BY `id_pesan` DESC";
